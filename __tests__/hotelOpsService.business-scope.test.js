...
describe('HotelOpsService business scoping', () => {
    ...
    test('rejects room status updates outside the business', async () => {
        ...
        await expect(svc.updateRoomStatus('room-b', 'MAINTENANCE', null, bpA)).rejects.toThrow('Room not found.');
    ...
    test('rejects housekeeping assignment when employee belongs to another business', async () => {
        const prisma = { businessEmployee: { findFirst: jest.fn().mockResolvedValue(null) }, hotelHousekeepingTask: { findFirst: jest.fn(), update: jest.fn() } };
        const svc = new HotelOpsService(prisma);
        await expect(svc.assignHousekeepingTask('task-b', 'employee-b', bpA)).rejects.toThrow('Employee not found.');
    ...
    test('rejects housekeeping assignment when task belongs to another business', async () => {
        const prisma = {
            businessEmployee: { findFirst: jest.fn().mockResolvedValue({ id: 'employee-a', businessProfileId: bpA, role: 'HOUSEKEEPER', userId: 1 }) },
            hotelHousekeepingTask: { findFirst: jest.fn().mockResolvedValue(null), update: jest.fn() },
        };
        const svc = new HotelOpsService(prisma);
        await expect(svc.assignHousekeepingTask('task-b', 'employee-a', bpA)).rejects.toThrow('Task not found.');
        expect(prisma.hotelHousekeepingTask.update).not.toHaveBeenCalled();
    });

    test('rejects checklist changes outside the business', async () => {
        const prisma = { hotelHousekeepingTask: { findFirst: jest.fn().mockResolvedValue(null), update: jest.fn() } };
        const svc = new HotelOpsService(prisma);
    ...
    test('rejects room blocks for foreign rooms', async () => {
        const prisma = { hotelRoom: { findFirst: jest.fn().mockResolvedValue(null) }, hotelRoomBlock: { create: jest.fn() } };
        const svc = new HotelOpsService(prisma);
        await expect(svc.blockRoom('room-b', { startDate: '2026-09-04', endDate: '2026-09-05', reason: 'maintenance' }, bpA)).rejects.toThrow('Room not found.');
        expect(prisma.hotelRoomBlock.create).not.toHaveBeenCalled();
    });

    test('rejects deletion of a foreign room block', async () => {
        const prisma = { hotelRoomBlock: { findFirst: jest.fn().mockResolvedValue(null), delete: jest.fn() } };
        const svc = new HotelOpsService(prisma);
    ...
    test('requires a registered Azaman customer for walk-in bookings', async () => {
        ...
        const svc = new HotelOpsService(prisma);
        await expect(svc.createWalkIn(bpA, { roomId: 'room-a', nights: 1 })).rejects.toThrow('A registered Azaman customer is required for a walk-in booking.');
        expect(prisma.hotelRoom.findFirst).not.toHaveBeenCalled();
    });

    test('resolves a walk-in customer by public Azaman ID', async () => {
        const prisma = {
            hotelRoom: { findFirst: jest.fn().mockResolvedValue({ id: 'room-a', status: 'AVAILABLE', basePriceUsdc: 100 }), update: jest.fn() },
        ...
        const svc = new HotelOpsService(prisma);
        await expect(svc.createWalkIn('business-a', { customerId: 42, roomId: 'room-a', nights: 1 })).resolves.toEqual({ id: 'reservation-a' });
    ...
    test('rejects room moves across business boundaries', async () => {
        const prisma = { reservation: { findFirst: jest.fn().mockResolvedValue(null) }, hotelRoom: { findFirst: jest.fn(), update: jest.fn() }, $transaction: jest.fn() };
        const svc = new HotelOpsService(prisma);
        await expect(svc.moveRoom('reservation-b', { newRoomId: 'room-b' }, bpA)).rejects.toThrow('Reservation not found');
        expect(prisma.$transaction).not.toHaveBeenCalled();
    });

    test('moves a same-business reservation atomically', async () => {
        const reservation = {
            id: 'reservation-a', businessProfileId: bpA, serviceItemId: 'room-old',
