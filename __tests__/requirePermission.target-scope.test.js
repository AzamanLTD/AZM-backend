...
describe('requirePermission target business scoping', () => {
    function makeReq({ key, id, businessProfileId = 'business-a', userRole = 'BUSINESS' }) {
        const prisma = {
            businessProfile: {
                findFirst: jest.fn().mockResolvedValue({ id: businessProfileId, userId: 99 }),
            },
            businessEmployee: {
                findUnique: jest.fn().mockResolvedValue({ permissions: ['*'], status: 'ACTIVE', role: 'MANAGER' }),
            },
            shift: { findFirst: jest.fn().mockResolvedValue({ id: id || 'target' }) },
            shiftSwap: { findFirst: jest.fn().mockResolvedValue({ id: id || 'target' }) },
        };
        const req = {
        ...
        return { req, prisma, middleware: requirePermission(key) };
    }
    ...
    test.each([
        ...
        expect(model.findFirst).toHaveBeenCalledWith({
            where: { id: 'target', businessProfileId: 'business-a' },
            select: { id: true },
        });
