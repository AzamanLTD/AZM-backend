...
describe('PayrollService disbursement integrity', () => {
        ...
        const tx = {
            ...
            user: {
                update: jest.fn().mockImplementation(async () => {
                    if (failureAt === 'user') throw new Error('balance write failed');
                    return { id: 'user-a' };
                }),
            },
            transactionHistory: {
                create: jest.fn().mockImplementation(async () => {
                    if (failureAt === 'history') throw new Error('history write failed');
                    return { id: 'history-a' };
                }),
            },
            businessLedgerEntry: {
                create: jest.fn().mockImplementation(async () => {
                    if (failureAt === 'ledger') throw new Error('ledger write failed');
                    return { id: 'ledger-a' };
                }),
            },
            businessEmployee: {
                update: jest.fn().mockImplementation(async () => {
                    if (failureAt === 'employee') throw new Error('employee reset failed');
    ...
    test('settles direct payroll and all accounting state in one serializable transaction', async () => {
        ...
        const result = await svc.disbursePayroll('payroll-a');

        expect(prisma.$transaction).toHaveBeenCalledTimes(1);
        expect(prisma.tx.user.update).toHaveBeenCalledWith({
            where: { id: 'user-a' },
            data: { azmBalance: { increment: 100 } },
        });
        ...
        expect(prisma.tx.businessEmployee.update).toHaveBeenCalledWith({
            where: { id: 'employee-a' },
            data: { accruedWages: 0.0, withdrawnEarly: 0.0 },
        });
        expect(prisma.tx.payrollRecord.update).toHaveBeenCalledWith({
            where: { id: 'payroll-a' },
            data: {
                status: 'PROCESSED',
    ...
    test('leaves payroll pending on settlement failure instead of recording a false FAILED/paid state', async () => {
        const prisma = buildPrisma({ failureAt: 'ledger' });
        const svc = new PayrollService(prisma);

        await expect(svc.disbursePayroll('payroll-a')).rejects.toThrow('ledger write failed');
    ...
    test('refuses to mark Smart Route payroll paid without an exact settlement worker', async () => {
        ...
        await expect(svc.disbursePayroll('payroll-a')).rejects.toThrow(
            'Payroll with Smart Route requires the payroll settlement worker; it was not marked as paid.',
        );
