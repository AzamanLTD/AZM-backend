...
describe('PayrollService Serializable retry', () => {
    test('retries a transient P2034 disbursement conflict and succeeds', async () => {
        const payroll = { id: 'payroll-1', businessProfileId: 'business-a', employeeId: 'employee-1', userId: 101, status: 'PENDING', netAmount: 100, employee: { businessProfileId: 'business-a', smartRouteId: null }, period: '2026-09' };
        const tx = {
            payrollRecord: { findFirst: jest.fn().mockResolvedValue(payroll), update: jest.fn().mockResolvedValue({ ...payroll, status: 'PROCESSED' }) },
        ...
        const prisma = { $transaction: jest.fn().mockRejectedValueOnce(conflict).mockImplementationOnce(async (callback, options) => { expect(options).toEqual({ isolationLevel: 'Serializable' }); return callback(tx); }) };
