...
describe('businessOrderService.getBusinessStats', () => {
    ...
    test('returns a complete 30-day revenue series independently of order-list pagination and includes authoritative invoice KPIs', async () => {
    ...
    test('fails when an authoritative invoice aggregate query fails', async () => {
        jest.useFakeTimers().setSystemTime(new Date('2026-09-03T15:30:00.000Z'));
        ...
        const prisma = {
            ...
            businessInvoice: {
                count: jest.fn().mockResolvedValue(0),
                aggregate: jest.fn().mockRejectedValue(new Error('invoice stats unavailable')),
            },
            $queryRaw: jest.fn().mockResolvedValue([]),
        };

        await expect(getBusinessStats(prisma, { businessProfileId: 'biz-1' }))
            .rejects.toThrow('invoice stats unavailable');
    });
});