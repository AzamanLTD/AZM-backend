...
describeOrSkip('BusinessOrder — lifecycle', () => {
    ...
    test('concurrent markDelivered calls allow exactly one PAID → DELIVERED transition', async () => {
        ...
        const customer = await seedUser(prisma);
        const order = await prisma.businessOrder.create({
            data: { businessProfileId: biz.id, customerId: customer.id, productId: product.id, status: 'PAID', title: 'Concurrent Order', amountUsdc: 50.0, orderRef: `ORD-CONCURRENT-${Date.now()}` },
        });
    ...
    test('late escrow FUNDED event cannot regress a refunded order', async () => {
        const { biz, product } = await seedBusiness(prisma);
        const customer = await seedUser(prisma);
        const ticket = await prisma.ticket.create({ data: { creatorId: customer.id, counterpartyId: biz.userId, name: 'Test', type: 'ESCROW', targetAmount: 50, targetCurrency: 'USDC', status: 'OPEN', businessProfileId: biz.id, lastActivityAt: new Date() } });
        const escrow = await prisma.smartEscrow.create({ data: { ticketId: ticket.id, payerId: customer.id, payeeId: biz.userId, amountUsdc: 50, feeUsdc: 0.5, status: 'DRAFT' } });
    ...
    test('business order pagination is deterministic for equal timestamps', async () => {
        const { biz } = await seedBusiness(prisma);
        const customer = await seedUser(prisma);
        ...
        expect(new Set(first.orders.map(order => order.id))).not.toEqual(new Set(second.orders.map(order => order.id)));
        await expect(businessOrderService.listOrdersForBusiness(prisma, { businessProfileId: biz.id, cursor: 'missing-order' })).rejects.toThrow(/cursor must reference an existing order/i);
