// __tests__/ledger-asset-identity.test.js
// =============================================================================
// §P.5-A — multi-asset ledger accounting identity (real PostgreSQL proofs).
//
// P4 balanced the journal numerically; with GHS and USDC both becoming
// authoritative that is no longer sufficient — 1200 GHS is not 100 USDC even
// when the columns sum equal. Proofs (per the §P.5-A contract):
//   A. existing P4 USDC postings remain valid (no behavior change)
//   B. same-asset multi-line postings balance normally
//   C. cross-asset numeric coincidence is rejected — deterministic, fail
//      closed, nothing committed
//   D. an explicit ASSET_CONVERSION represents a legitimate cross-asset
//      exchange: durable exactly-once conversion identity, exact rate
//      provenance, asset-specific legs that EACH balance within their asset
//   E. a cross-asset failure rolls back the ENTIRE enclosing transaction
//   F. a persisted account whose identity disagrees with the chart fails closed
// No inventory, GHS customer liability, spread or P&L is invented: the GHS
// leg uses the catalog-only equity:treasury:ghs counter (equity-mediated
// exchange proof), not Model B settlement semantics.
// =============================================================================
const hasDb = !!process.env.TEST_DATABASE_URL;
const describeOrSkip = hasDb ? describe : describe.skip;
if (!hasDb) console.warn('[ledger-asset-identity] TEST_DATABASE_URL not set — skipping DB proofs.');
...
describeOrSkip('§P.5-A multi-asset accounting identity (real PostgreSQL)', () => {
    ...
    // GHS↔USDC exchange mechanism proof: two asset legs, each balanced within
    // its own asset (GHS received against GHS equity / USDC delivered against
    // USDC equity). The rate is provenance, never a numeric equality.
    ...
    describe('A/B. USDC postings remain valid (P4 compatibility)', () => {
        ...
        it('a many-line USDC posting (3 accounts, same asset) balances normally', async () => {
            const user = await seedUser(prisma);
            await prisma.$transaction(async (tx) => post(tx, {
                idempotencyKey: 'p5a:usdc:multi',
                entryType: 'FEE',
                description: 'multi-line USDC split',
                lines: [
                    { account: `user:${user.id}:liability`, debit: '10' },
                    { account: 'revenue:fees', credit: '6' },
                    { account: 'revenue:spread', credit: '4' },
                ],
            }));
            expect((await ledger.accountBalance(prisma, 'revenue:fees')).balance.toFixed(8)).toBe('6.00000000');
            expect((await ledger.accountBalance(prisma, 'revenue:spread')).balance.toFixed(8)).toBe('4.00000000');
        });
    });
    ...
    describe('E. a cross-asset failure rolls back the ENTIRE enclosing transaction', () => {
        it('a mixed-asset posting inside a financial mutation leaves NO partial mutation', async () => {
