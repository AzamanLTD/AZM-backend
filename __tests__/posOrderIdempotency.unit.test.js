...
function fingerprint(intent) {
    const canonical = {
        ...
        items: [...intent.items].map(({ productId, quantity }) => ({ productId: String(productId), quantity: Number(quantity) }))
...
describe('PosOrderService idempotency intent binding', () => {
    ...
    test('binds the fingerprint into the POS ledger entry when creating a new order', async () => {
        const tx = {
            businessOrder: {
            ...
            user: {
                ...
                findUnique: jest.fn().mockResolvedValue({ id: 7, azmBalance: 10 }),
            },
