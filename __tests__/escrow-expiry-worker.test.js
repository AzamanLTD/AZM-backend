...
describe('EscrowExpiryWorker refund convergence', () => {
    ...
    test('runs the canonical refund and balance convergence only after refund succeeds', async () => {
        const escrow = {
            id: 'escrow-1',
            ticketId: 'ticket-1',
            payerId: 11,
            payeeId: 22,
            ticket: { id: 'ticket-1', status: 'OPEN', creatorId: 11, counterpartyId: 22 },
        };
        const refunded = { id: 'escrow-1', ticketId: 'ticket-1', status: 'EXPIRED', amountUsdc: 50 };
        const emit = jest.fn();
        ...
        const prisma = {
            ...
            ticketMessage: { create: jest.fn().mockResolvedValue({ id: 'message-1', type: 'SYSTEM' }) },
        };
        ...
        expect(emit).not.toHaveBeenCalledWith('escrow_refunded', expect.anything());
    ...
    test('does not emit financial convergence when the refund transaction fails', async () => {
        const escrow = { id: 'escrow-2', ticketId: 'ticket-2', payerId: 31, payeeId: 32, ticket: null };
        const emit = jest.fn();
        const io = { to: jest.fn(() => ({ emit })) };
        const emitBalanceUpdate = jest.fn();
        const prisma = { smartEscrow: { findMany: jest.fn().mockResolvedValue([escrow]) } };
