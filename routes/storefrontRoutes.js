...
const storefrontAtomicService = require('../services/storefrontAtomicService');
...
router.get('/discover', wrap(async (req, res) => {
  ...
  const [layouts, total] = await Promise.all([
    prisma.businessStorefrontLayout.findMany({ where, include: { businessProfile: { select: { id: true, bizId: true, businessName: true, category: true, logoUrl: true, coverPhotoUrl: true, averageRating: true, reviewCount: true, description: true, address: true, phoneNumber: true } }, theme: { select: { key: true, name: true, tokenSet: true } } }, orderBy: { publishedAt: 'desc' }, take, skip }),
...
router.put('/me/draft', protect, protectActive, requirePermission('storefront.manage'), wrap(async (req, res) => {
  ...
  res.json({ success: true, data: await storefrontAtomicService.saveDraft(prisma, businessProfileId, parsed.data.layoutJson, parsed.data.themeId, parsed.data.expectedUpdatedAt) });
}));
router.post('/me/publish', protect, protectActive, requirePermission('storefront.manage'), wrap(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten() });
  const published = await storefrontAtomicService.publishLayout(prisma, businessProfileId, req.user.id, parsed.data.expectedUpdatedAt);
...
router.post('/me/revert', protect, protectActive, requirePermission('storefront.manage'), wrap(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten() });
  res.json({ success: true, data: await storefrontAtomicService.revertToVersion(prisma, businessProfileId, parsed.data.versionId, parsed.data.expectedUpdatedAt), message: 'Reverted to selected version. Review and publish.' });
}));
router.post('/me/apply-template', protect, protectActive, requirePermission('storefront.manage'), wrap(async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ success: false, message: 'Validation failed', errors: parsed.error.flatten() });
  res.json({ success: true, data: await storefrontAtomicService.applyTemplate(prisma, businessProfileId, parsed.data.templateId, parsed.data.expectedUpdatedAt), message: 'Template applied. Review and publish.' });
}));
