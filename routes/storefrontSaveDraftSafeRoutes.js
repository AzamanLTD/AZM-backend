...
function wrap(handler) {
  return async (req, res) => {
    try { await handler(req, res); }
      ...
      res.status(err.statusCode || 400).json({ success: false, message: err.message, ...(err.code ? { code: err.code } : {}) });
    }
  };
}
...
router.put('/me/draft', protect, protectActive, requirePermission('storefront.manage'), wrap(async (req, res) => {
  ...
  const draft = await saveDraftSafe(prisma, businessProfileId, parsed.data.layoutJson, parsed.data.themeId, parsed.data.expectedUpdatedAt);
