...
router.post('/groups',                    protectActive, idempotency(), ctrl.createSusu);
router.get('/groups/:id',                 protect,       ctrl.getDetail);
router.get('/groups/:id/transparency',    protect,       ctrl.getTransparencyReport);
router.post('/groups/:id/contract',       protectActive, idempotency(), ctrl.acceptContract);
router.post('/groups/:id/cancel',         protectActive, idempotency(), ctrl.cancel);
router.get('/groups/:id/my-position',     protect,       ctrl.myPosition);

router.post('/vouches',                   protectActive, idempotency(), ctrl.submitVouch);
...
router.post('/',                                      protectActive, idempotency(), overlay.createSusu);
...
router.post('/:id/cancel',                            protectActive, idempotency(), overlay.cancelSusu);
...
router.post('/invites/:token/redeem',                 protectActive, idempotency(), overlay.redeemInvite);
...
router.post('/:id/contract/accept',                   protectActive, idempotency(), overlay.acceptContract);
