 ...
 */
async function startWorkers(app, { prisma, io, workerStatus, tradeSocketService, mtnDisbursementService, paymentFailoverService, emailService, smsService, notificationService, vaultService, susuService, smartRouteService, azmAuctionService, susuInitiationService }) {
    ...
    const register = async (name, schedule, handler, opts) => { if (!IS_TEST_ENV) await scheduler.register(name, schedule, handler, opts); };
    ...
    // Financial truth: create an immutable reserve/liability commitment once
    // per hour. Public PoR reads only return the latest snapshot.
    ...
    await register('no-show-sweep', '0 * * * *', async () => { const results = await noShowWorker.sweepAll(prisma); logger.info({ results }, 'NoShowWorker: sweep complete'); });
    ...
    await register('webhook-retry', '*/2 * * * *', async () => { const count = await webhookDispatcher.processRetryQueue(); if (count > 0) logger.info({ count }, 'WebhookRetry: processed stuck deliveries'); });
    const webhookController = require('../../controllers/webhookController');
    await register('webhook-delivery-retry', '*/2 * * * *', async () => { await webhookController.processRetries(prisma); });
    ...
    if (!IS_TEST_ENV) {
        ...
        const startV2Workers = async () => {
            const start = Date.now(); const MAX_WAIT_MS = 30 * 60 * 1000;
            while (!app.get('azamanTreasuryUserId') && Date.now() - start < MAX_WAIT_MS) await new Promise(r => setTimeout(r, 5000));
            const treasuryUserId = app.get('azamanTreasuryUserId');
            if (!treasuryUserId) { logger.warn('SusuV2 Workers: treasury cache not resolved within 30 min. Cycle/reminder/PoR workers NOT started.'); return; }
            const susuCycleService = new SusuCycleService(prisma, { susuVouchService: app.get('susuVouchService'), susuMemberService: app.get('susuMemberService'), adminWarRoomService: app.get('adminWarRoomService'), notificationService, io, treasuryUserId });
            ...
            app.set('susuCycleSchedulerV2', cycleSchedulerV2); app.set('susuReminderCron', reminderCron); app.set('porExpirySweep', porExpirySweep);
        };
    ...
    if (!IS_TEST_ENV) {
        const gracefulShutdown = async () => { await scheduler.closeAll(); process.exit(0); };
        process.on('SIGTERM', gracefulShutdown); process.on('SIGINT', gracefulShutdown);
    }
    Object.assign(workerStatus, { leaderboardWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', analyticsWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', cfoWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', savingsWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', withdrawalReconciliationWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', payoutBatchWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', vaultWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', susuWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', smartRouteWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', azmAuctionWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', onchainSweepWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', disappearingMessageWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', proofOfReservesWorker: IS_TEST_ENV ? 'disabled_in_test' : 'running', tradeWorker: IS_TEST_ENV ? 'disabled_in_test' : 'pending_listen' });
