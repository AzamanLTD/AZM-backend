...
class PosOrderService {
    ...
    async createOrder(args) {
        const { businessProfileId, actorId, items, paymentMethod = 'CASH', cashGiven, azmAmount, idempotencyKey, source, locationId, tableId, customerId } = args;
        ...
        for (const item of normalizedItems) if (!item.productId || !Number.isInteger(item.quantity) || item.quantity < 1 || item.quantity > 50) throw new Error('Each item requires a valid productId and integer quantity from 1 to 50.');
        ...
        let azmPortion = 0;
        let cashChange = 0;
        if (pm === 'CASH') { if (cash < computedGrand) throw new Error('Insufficient cash received.'); cashChange = cash - computedGrand; }
        else if (pm === 'AZM') azmPortion = computedGrand;
        else { azmPortion = requestedAzm; if (!Number.isFinite(azmPortion) || azmPortion <= 0 || azmPortion > computedGrand) throw new Error('Invalid AZM portion.'); if (cash + azmPortion < computedGrand) throw new Error('Insufficient payment (cash + AZM).'); cashChange = Math.max(0, cash - (computedGrand - azmPortion)); }
            ...
            try {
                const result = await this.prisma.$transaction(async (tx) => {
                    if (idempotencyKey) {
                        ...
                        if (txExisting) { if (txExisting.businessProfileId !== businessProfileId) throw new Error('Idempotency key already belongs to another business.'); return { order: txExisting, duplicate: true }; }
                    }
                    if (azmPortion > 0) {
                    ...
                    const order = await tx.businessOrder.create({ data: { businessProfileId, customerId: effectiveCustomerId, status: 'COMPLETED', orderRef, title: `POS Sale (${pm})`, amountUsdc: computedGrand, paymentMethod: pm, idempotencyKey, cashReceived: cash || null, cashChange: cashChange || null, completedAt: new Date() } });
                    await tx.businessLedgerEntry.create({ data: { businessProfileId, type: 'INCOME', category: 'SALES', description: `POS Sale (${orderRef} - ${pm})`, amount: computedGrand, sourceType: 'POS_SALE', sourceId: order.id, metadata: { orderRef, paymentMethod: pm, items: normalizedItems.length, locationId, tableId, azmPortion } } });
                ...
                if (error?.code === 'P2002' && idempotencyKey) { const replay = await this._findIdempotentOrder(businessProfileId, idempotencyKey); if (replay) return { order: replay, duplicate: true, computedSubtotal: computed.subtotal, computedTax, computedGrand, change: Number(replay.cashChange || 0) }; }
    ...
    async _priceItems(businessProfileId, items) {
        ...
        for (const item of items) {
            const product = await this.prisma.businessProduct.findFirst({ where: { id: item.productId, businessProfileId, isActive: true, isAvailable: true }, select: { id: true, name: true, priceUsdc: true } });
            ...
            subtotal += price * item.quantity; priced.push({ ...item, name: product.name, unitPrice: price });
        }
