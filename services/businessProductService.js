...
const _cleanOptionRows = (value, label, max) => {
    ...
    return value.map((row, index) => {
        ...
        if (name.length < 1 || name.length > 100) throw new Error(`${label}[${index}].name must be 1–100 chars.`);
        const priceDelta = Number(row.priceDelta ?? 0);
        if (!Number.isFinite(priceDelta)) throw new Error(`${label}[${index}].priceDelta must be finite.`);
...
const _cleanModifierGroups = (value) => {
    ...
    return value.map((group, index) => {
        if (!group || typeof group !== 'object' || Array.isArray(group)) throw new Error(`modifierGroups[${index}] must be an object.`);
        const name = String(group.name ?? '').trim();
        if (name.length < 1 || name.length > 100) throw new Error(`modifierGroups[${index}].name must be 1–100 chars.`);
        const maxSelection = Number(group.maxSelection ?? 1);
        if (!Number.isInteger(maxSelection) || maxSelection < 1 || maxSelection > 20) throw new Error(`modifierGroups[${index}].maxSelection must be an integer from 1 to 20.`);
        ...
        return {
            ...
            options: options.map((option, optionIndex) => {
                if (!option || typeof option !== 'object' || Array.isArray(option)) throw new Error(`modifierGroups[${index}].options[${optionIndex}] must be an object.`);
                const optionName = String(option.name ?? '').trim();
                if (optionName.length < 1 || optionName.length > 100) throw new Error(`modifierGroups[${index}].options[${optionIndex}].name must be 1–100 chars.`);
                const priceDelta = Number(option.priceDelta ?? 0);
                if (!Number.isFinite(priceDelta)) throw new Error(`modifierGroups[${index}].options[${optionIndex}].priceDelta must be finite.`);
...
const _validateFields = ({ name, description, priceUsdc, imageUrls, category, catalogSectionId, tags, calorieCount, preparationMins, variants, modifierGroups, locationId, deliveryTerms, estimatedDelivery, isAvailable }, { partial }) => {
    const data = {};
    ...
    if (imageUrls !== undefined) {
        if (!Array.isArray(imageUrls) || !imageUrls.every((u) => typeof u === 'string')) throw new Error('imageUrls must be an array of strings.');
    ...
    if (category !== undefined) {
        if (category != null && !VALID_CATEGORIES.has(category)) throw new Error(`category must be one of: ${[...VALID_CATEGORIES].join(', ')}`);
    ...
    if (tags !== undefined) {
        if (!Array.isArray(tags) || !tags.every((tag) => typeof tag === 'string')) throw new Error('tags must be an array of strings.');
    ...
    for (const [key, value] of [['calorieCount', calorieCount], ['preparationMins', preparationMins]]) {
        if (value !== undefined) {
            if (value === null || value === '') data[key] = null;
                ...
                if (!Number.isInteger(number) || number < 0 || number > 100000) throw new Error(`${key} must be a non-negative integer.`);
                data[key] = number;
            }
        }
    }
...
const _resolveLocation = async (prisma, businessProfileId, locationId) => {
    ...
    const location = await prisma.businessLocation.findUnique({ where: { id: locationId }, select: { id: true, businessProfileId: true } });
...
const _generateSlug = async (prisma, businessName, productName) => {
    const base = `${businessName}-${productName}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80);
...
const createProduct = async (prisma, args) => {
    const { businessProfileId, name, description, priceUsdc, imageUrls, category, catalogSectionId, tags, calorieCount, preparationMins, variants, modifierGroups, locationId, deliveryTerms, estimatedDelivery, isAvailable } = args;
    if (!businessProfileId) throw new Error('businessProfileId is required.');
    const profile = await prisma.businessProfile.findUnique({ where: { id: businessProfileId }, select: { id: true, businessName: true } });
    if (!profile) throw new Error('Business profile not found.');
    const data = _validateFields({ name, description, priceUsdc, imageUrls, category, catalogSectionId, tags, calorieCount, preparationMins, variants, modifierGroups, locationId, deliveryTerms, estimatedDelivery, isAvailable }, { partial: false });
    ...
    return prisma.businessProduct.create({ data: { businessProfileId, name: data.name, description: data.description ?? null, priceUsdc: data.priceUsdc, imageUrls: data.imageUrls ?? [], category: data.category ?? null, slug, catalogSectionId: resolvedSectionId, tags: data.tags ?? [], calorieCount: data.calorieCount ?? null, preparationMins: data.preparationMins ?? null, variants: data.variants ?? [], modifierGroups: data.modifierGroups ?? [], locationId: resolvedLocationId, deliveryTerms: data.deliveryTerms ?? null, estimatedDelivery: data.estimatedDelivery ?? null, isAvailable: data.isAvailable !== false } });
};

const updateProduct = async (prisma, { productId, businessProfileId, updates }) => {
    if (!productId) throw new Error('productId is required.');
    const product = await prisma.businessProduct.findUnique({ where: { id: productId }, include: { businessProfile: { select: { businessName: true } } } });
    if (!product) throw new Error('Product not found.');
    if (product.businessProfileId !== businessProfileId) throw new Error('You do not own this product.');
    const filtered = {};
    for (const [key, value] of Object.entries(updates || {})) if (UPDATABLE_FIELDS.has(key)) filtered[key] = value;
    ...
    if (data.name && data.name !== product.name) data.slug = await _generateSlug(prisma, product.businessProfile.businessName, data.name);
...
const listProducts = async (prisma, { businessProfileId, isActive, locationId, limit, cursor }) => {
    ...
    if (locationId) {
        // A selected branch may see global catalog entries plus products explicitly assigned to it.
...
const getProduct = async (prisma, { productId, includeBusinessProfile }) =>
    prisma.businessProduct.findUnique({
        where: { id: productId },
        ...(includeBusinessProfile ? { include: { businessProfile: { select: { bizId: true, businessName: true, isVerified: true, kybStatus: true } } } } : {}),
    });

const deleteProduct = async (prisma, { productId, businessProfileId }) => {
    if (!productId) throw new Error('productId is required.');
    const product = await prisma.businessProduct.findUnique({ where: { id: productId }, select: { id: true, businessProfileId: true } });
    if (!product) throw new Error('Product not found.');
    if (product.businessProfileId !== businessProfileId) throw new Error('You do not own this product.');
    const activeOrders = await prisma.businessOrder.count({ where: { productId, status: { notIn: ['COMPLETED', 'REFUNDED', 'CANCELLED'] } } });
