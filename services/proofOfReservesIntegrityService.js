...
function merkleRootFromHashes(hashes) {
  ...
  while (layer.length > 1) {
    const next = [];
    for (let i = 0; i < layer.length; i += 2) next.push(sha256(layer[i] + (i + 1 < layer.length ? layer[i + 1] : layer[i])));
    layer = next;
  }
  return layer[0];
}
function merkleProofFromHashes(hashes, index) {
  const proof = [];
  let layer = [...hashes]; let idx = index;
  while (layer.length > 1) {
...
async function loadState(tx) {
  const users = await tx.user.findMany({ where: { isDeleted: false }, select: { id: true, availableBalance: true, escrowLockedBalance: true, vendorUnallocatedBalance: true, disputeEscrowBalance: true }, orderBy: { id: 'asc' } });
  const [cryptoWallet, hotWallet, fiatPool] = await Promise.all([tx.systemMasterCrypto.findUnique({ where: { id: 1 } }), tx.systemHotWallet.findUnique({ where: { id: 1 } }), tx.systemFiatPool.findUnique({ where: { id: 1 } })]);
...
async function createSnapshot() {
  return prisma.$transaction(async tx => {
    const state = await loadState(tx); const salt = crypto.randomBytes(16).toString('hex');
    const leaves = state.users.map(u => ({ userId: u.id, availableBalance: u.availableBalance.toString(), escrowLockedBalance: u.escrowLockedBalance.toString(), vendorUnallocatedBalance: u.vendorUnallocatedBalance.toString(), disputeEscrowBalance: u.disputeEscrowBalance.toString(), leafHash: sha256(`${u.id}|${u.availableBalance}|${u.escrowLockedBalance}|${u.vendorUnallocatedBalance}|${u.disputeEscrowBalance}|${salt}`) }));
    const root = merkleRootFromHashes(leaves.map(l => l.leafHash)); const ratio = state.totalLiabilities > 0 ? state.totalReserves / state.totalLiabilities : 1;
    ...
    for (const leaf of leaves) await tx.$executeRaw`INSERT INTO "ProofOfReservesLeaf" ("snapshotId", "userId", "availableBalance", "escrowLockedBalance", "vendorUnallocatedBalance", "disputeEscrowBalance", "leafHash") VALUES (${snapshot.id}, ${leaf.userId}, ${leaf.availableBalance}, ${leaf.escrowLockedBalance}, ${leaf.vendorUnallocatedBalance}, ${leaf.disputeEscrowBalance}, ${leaf.leafHash})`;
...
async function getLatestSnapshot() {
  ...
  return { timestamp: snapshot.createdAt.toISOString(), totalLiabilities: snapshot.totalLiabilities.toString(), totalReserves: snapshot.totalReserves.toString(), reserveRatio: snapshot.reserveRatio.toString(), isFullyBacked: snapshot.isFullyBacked, userCount: snapshot.userCount, merkleRoot: snapshot.merkleRoot, breakdown: snapshot.breakdown };
}
...
async function getIntegrityReport() {
  ...
  const leafCount = Number(countRows[0]?.count || 0), coverage = latest.userCount === 0 ? 1 : leafCount / latest.userCount;
...
module.exports = { createSnapshot, getLatestSnapshot, verifyUser, getIntegrityReport };
